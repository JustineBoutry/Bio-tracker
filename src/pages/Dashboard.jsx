...
export default function Dashboard() {
  ...
  const getSporeLoadChartData = (filterByFacet = null) => {
    if (!experiment?.factors || selectedSporeLoadGraphFactors.length === 0) return { boxData: [], scatterData: [] };
    ...
    filteredInds = filteredInds.filter(ind => 
      ind.infected === "confirmed Yes" && ind.spores_count != null
    );
    ...
    filteredInds.forEach(ind => {
      ...
      const sporeCount = Number(ind.spores_count) || 0;
      groups[groupKey].values.push(sporeCount);
    });
    ...
    sortedGroups.forEach((group, groupIdx) => {
      group.values.forEach((value, i) => {
        scatterData.push({
          name: group.name,
          x: groupIdx + (Math.random() - 0.5) * 0.3,
          y: value,
          groupIdx
        });
      });
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                      ...
                      >
                        ...
                        <XAxis 
                          ...
                          interval={0}
                        />
                        <YAxis label={{ value: 'Mean Spore Count ± 95% CI', angle: -90, position: 'insideLeft' }} />
                        <Tooltip 
                          content={({ active, payload }) => {
                            if (active && payload && payload.length) {
                              const data = payload[0].payload;
                        ...
                        />
                        <Bar dataKey="mean" fill="#8b5cf6" name="Mean Spore Count">
                          <ErrorBar dataKey="ci95" width={4} strokeWidth={2} stroke="#6d28d9" />
                        </Bar>
                      </ComposedChart>
                    </ResponsiveContainer>
                    <div className="mt-4 overflow-x-auto">
