...
export default function Dashboard() {
  ...
  const [selectedOffspringPerDayGraphFactors, setSelectedOffspringPerDayGraphFactors] = useState([]);
  const [facetOffspringPerDayFactor, setFacetOffspringPerDayFactor] = useState(null);
  const [offspringPerDayByRedStatus, setOffspringPerDayByRedStatus] = useState(false);
  ...
  const getOffspringPerDayChartData = (filterByFacet = null) => {
    ...
    let filteredInds = filterByFacet 
      ? allIndividuals.filter(ind => ind.factors?.[facetOffspringPerDayFactor] === filterByFacet)
      : allIndividuals;
    ...
    filteredInds.forEach(ind => {
      let groupKey = selectedOffspringPerDayGraphFactors
        .map(factor => ind.factors?.[factor] || 'Unknown')
        .join(' - ');
      
      // Add red status to group key if enabled
      if (offspringPerDayByRedStatus) {
        const redStatus = ind.red_confirmed ? 'Red+' : 'Red-';
        groupKey = groupKey ? `${groupKey} | ${redStatus}` : redStatus;
      }
      ...
      let daysLived;
      if (ind.alive) {
        // Still alive: use days post infection if available, otherwise days since start
        if (experiment.infection_date) {
          daysLived = differenceInDays(new Date(), new Date(experiment.infection_date));
        } else {
          daysLived = differenceInDays(new Date(), new Date(experiment.start_date));
        }
      } else if (ind.death_date && ind.death_date !== 'lost') {
        // Dead: death date - start date
        daysLived = differenceInDays(new Date(ind.death_date), new Date(experiment.start_date));
      } else {
        // Lost or no death date - skip
        return;
      }
      
      // Skip if days lived is 0 or negative
      if (daysLived <= 0) return;
      
      const offspring = Number(ind.cumulative_offspring) || 0;
      const offspringPerDay = offspring / daysLived;
      groups[groupKey].values.push(offspringPerDay);
    });
  ...
  const offspringPerDayChartResult = !facetOffspringPerDayFactor ? getOffspringPerDayChartData() : null;
  const offspringPerDayFacetLevels = getOffspringPerDayFacetLevels();
